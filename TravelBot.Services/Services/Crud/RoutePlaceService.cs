...
public class RoutePlaceService : IRoutePlaceService, IServiceAnchor
{
    ...
    ///     ctor
    /// </summary>
    public RoutePlaceService(IRoutePlaceReadRepository routePlaceReadRepository,
    ...
    async Task<RoutePlaceModel> IRoutePlaceService.Create(RoutePlaceCreateModel model,
    ...
    async Task<RoutePlaceModel> IRoutePlaceService.Edit(Guid id, RoutePlaceCreateModel model,
        CancellationToken cancellationToken)
    {
        var item = await routePlaceReadRepository.GetById(id, cancellationToken)
                   ?? throw new TravelBotNotFoundException($"Не удалось найти маршрут-место с идентификатором {id}");
