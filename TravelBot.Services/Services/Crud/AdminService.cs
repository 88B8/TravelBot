...
public class AdminService : IAdminService, IServiceAnchor
{
    ...
    ///     ctor
    /// </summary>
    public AdminService(IAdminReadRepository adminReadRepository, IAdminWriteRepository adminWriteRepository,
    ...
    async Task<AdminModel> IAdminService.GetByLogin(string login, CancellationToken cancellationToken)
    {
        var item = await adminReadRepository.GetByLogin(login, cancellationToken)
                   ?? throw new TravelBotNotFoundException($"Не удалось найти администратора с логином {login}");
