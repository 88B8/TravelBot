...
public class UserService : IUserService, IServiceAnchor
{
    ...
    public UserService(IUserReadRepository userReadRepository, IUserWriteRepository userWriteRepository,
        IPassportReadRepository passportReadRepository, IPassportWriteRepository passportWriteRepository,
    ...
    async Task<UserModel> IUserService.Edit(Guid id, UserCreateModel model, CancellationToken cancellationToken)
    {
        ...
        var passport = await passportReadRepository.GetByIdRaw(model.PassportId, cancellationToken)
                       ?? throw new TravelBotNotFoundException(
                           $"Не удалось найти паспорт с идентификатором {model.PassportId}");
    ...
    async Task<UserModel> IUserService.GetByTelegramId(long telegramId, CancellationToken cancellationToken)
    {
        var item = await userReadRepository.GetByTelegramId(telegramId, cancellationToken)
                   ?? throw new TravelBotNotFoundException(
                       $"Не удалось найти пользователя с идентификатором {telegramId}");
