...
namespace TravelBot.Services.Services
{
    ...
    public class PassportPlaceService : IPassportPlaceService, IServiceAnchor
    {
        ...
        /// ctor
        /// </summary>
        public PassportPlaceService(IPassportPlaceReadRepository passportPlaceReadRepository, IPassportPlaceWriteRepository passportPlaceWriteRepository, IUnitOfWork unitOfWork, IMapper mapper)
        {
        ...
        async Task<PassportPlaceModel> IPassportPlaceService.Create(PassportPlaceCreateModel model, CancellationToken cancellationToken)
        {
        ...
        async Task<PassportPlaceModel> IPassportPlaceService.Edit(Guid id, PassportPlaceCreateModel model, CancellationToken cancellationToken)
        {
            var item = await passportPlaceReadRepository.GetById(id, cancellationToken)
                       ?? throw new TravelBotNotFoundException($"Не удалось найти место с идентификатором {id}");
        ...
        async Task<IReadOnlyCollection<PassportPlaceModel>> IPassportPlaceService.GetAll(CancellationToken cancellationToken)
        {
