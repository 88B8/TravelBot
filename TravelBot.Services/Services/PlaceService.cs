...
namespace TravelBot.Services.Services
{
    ...
    public class PlaceService : IPlaceService, IServiceAnchor
    {
        ...
        /// ctor
        /// </summary>
        public PlaceService(IPlaceReadRepository placeReadRepository, IPlaceWriteRepository placeWriteRepository, ICategoryReadRepository categoryReadRepository, IUnitOfWork unitOfWork, IMapper mapper)
        {
        ...
        async Task<PlaceModel> IPlaceService.Edit(Guid id, PlaceCreateModel model, CancellationToken cancellationToken)
        {
            ...
            var category = await categoryReadRepository.GetById(model.CategoryId, cancellationToken)
                        ?? throw new TravelBotNotFoundException($"Не удалось найти категорию с идентификатором {model.CategoryId}");
