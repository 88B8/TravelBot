...
public abstract class BaseWriteRepository<T> : IDbWriter<T> where T : class
{
    ...
    {
        if (entity is IEntityWithAudit auditCreated) auditCreated.CreatedAt = dateTimeProvider.UtcNow();
    }

    private void UpdateAudit([NotNull] T entity)
    {
        if (entity is IEntityWithAudit auditCreated) auditCreated.UpdatedAt = dateTimeProvider.UtcNow();
    }
}